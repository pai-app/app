...
export type TagItemProps = {
  ...
  /** Preloaded icon map. Icons render only after the pack has finished loading. */
 ...
 */
export function TagItem({ tag, onSelect, selectedTagId, icons }: TagItemProps) {
  const isRemove = tag.id === REMOVE_TAG_ID
  const isSelected = selectedTagId === tag.id
  const ParentIcon = icons?.[tag.icon]
  ...
  return (
    ...
    >
      ...
      >
        {ParentIcon && <ParentIcon className="m-1 size-6 min-w-6" />}
        ...
        <div className="flex flex-row gap-1 overflow-x-auto scrollbar-none">
          {tag.children.map((child) => {
            ...
            return (
              ...
              >
                {ChildIcon && <ChildIcon className="size-6" />}
